...
import { FileText, Eye, Printer, User, MapPin, Calendar, DollarSign } from 'lucide-react'
import { formatDate } from '@/lib/utils'

export const dynamic = 'force-dynamic'

export default async function ReportsPage() {
  // Fetch completed inspections with their quotes
      ...
      {completedInspections.length === 0 ? (
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-xs overflow-hidden">
          <div className="p-6 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
            ...
            <span className="text-xs text-neutral-400">{completedInspections.length} informe{completedInspections.length !== 1 ? 's' : ''}</span>
          </div>
              ...
              >
                  ...
                  {insp.quote && (
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-xs font-semibold text-teal-600 dark:text-teal-400">
                        Cotización: ${Math.round(insp.quote.totalAmount * 1.19).toLocaleString('es-CL')} c/IVA
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  {insp.quote ? (
                    ...
                    <Link
                      href={`/inspections/${insp.id}/quote`}
                      className="flex items-center justify-center gap-1.5 px-3.5 py-2 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 rounded-xl text-xs font-semibold border border-amber-100/10 transition-colors"
                    >
